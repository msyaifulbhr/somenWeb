...
const handleRadioClick = (event) => {
  ...
  if (clickedOption.checked) {
    const soal3 = localStorage.getItem('soal' + questionNumber);
...
const Soal3 = {
  ...
  async afterRender() {
    ...
    fetchQuestionData(3);
    var selectedOption = localStorage.getItem('soal3');
  if (selectedOption) {
    document.getElementById('q3_option' + selectedOption).checked = true;
  }
      var score2 = localStorage.getItem('soal2');
      if (!score2) {
        // Jika skor untuk pertanyaan sebelumnya tidak ada, redirect ke back.html
        window.location.href = '#/back';
      }
       const radioButtons = document.querySelectorAll('input[name=q3]');
